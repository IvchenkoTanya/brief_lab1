...
<label for="communication_format">7. Якому формату спілкування Ви віддаєте перевагу?</label><br>
...
<script>
    const divMsg = document.getElementById('errorMsg')

const getFields = (evt) => {
    const formData = new FormData(evt.target);
    const fields = {};
    formData.forEach((value, key) => {
        key = key.replace(/\[\]$/, ''); // Видаляємо квадратні дужки, якщо вони є
        if (fields[key]) {
    ...
    const removeEll = (e) => {
        while (divMsg.hasChildNodes()) {
            divMsg.removeChild(divMsg.firstChild);
        }
    }

    document.getElementById('briefInfo').addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: "/update.php",
            method: "POST",
            data: {'params': fields, 'id': <?php echo $_GET['id']; ?>},
            success: function (item) {
                ...
                if (msg['message'] == true) {
                    ...
                    div.appendChild(document.createTextNode("Дані збережено!"))
                    divMsg.appendChild(div)
                    setTimeout(() => removeEll(), 2000);
                    setTimeout(() =>  window.location.href = "https://labipz.000webhostapp.com/allbriefs.php", 1000);


                } else {
                    const div = document.createElement('div')
                    div.classList += "alert alert-danger";
                    div.appendChild(document.createTextNode("Сталася помилка!"))
                    divMsg.appendChild(div)
                    setTimeout(() => removeEll(), 2000);
                }
            }
